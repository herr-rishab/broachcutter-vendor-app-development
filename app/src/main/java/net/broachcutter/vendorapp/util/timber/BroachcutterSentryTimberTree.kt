 ...
 *
 * Modified version of [io.sentry.android.timber.SentryTimberTree]
 */
class BroachcutterSentryTimberTree(
    ...
    /**
     * do not log if it's lower than min. required level or in our list of error codes that don't need
     * to be logged.
     */
    private fun isLoggable(
        level: SentryLevel,
        ...
        return if (throwable == null || throwable !is AppException) {
            level.ordinal >= minLevel.ordinal
        } else {
            !allowedErrors.contains(throwable.errorCode)
        }
    }
    ...
    private fun captureEvent(
        ...
        if (isLoggable(sentryLevel, minEventLevel, throwable)) {
            val sentryEvent = SentryEvent().apply {
